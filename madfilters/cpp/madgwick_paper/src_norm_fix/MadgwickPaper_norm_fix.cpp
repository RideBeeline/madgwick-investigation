...
#include "MadgwickPaper_norm_fix.h"

namespace fixed {
// Math library required for ‘sqrt’
#include <math.h>     // System constants
#define deltatDef 0.001f // sampling period in seconds (shown as 1 ms)
#define gyroMeasError                                                          \
  3.14159265358979 *                                                           \
      (5.0f /                                                                  \
       180.0f) // gyroscope measurement error in rad/s (shown as 5 deg/s)
#define gyroMeasDrift                                                          \
  3.14159265358979 * (0.2f / 180.0f) // gyroscope measurement error in rad/s/s
                                     // (shown as 0.2f deg/s/s)
#define betaDef sqrt(3.0f / 4.0f) * gyroMeasError // compute beta
#define zetaDef                                                                \
  sqrt(3.0f / 4.0f) * gyroMeasDrift // compute zeta// Global system variables

float deltat = deltatDef;
float beta = betaDef;
float zeta = zetaDef;
...
void filterUpdate(float w_x, float w_y, float w_z, float a_x, float a_y,
  ...
  float SEqDot_omega_1, SEqDot_omega_2, SEqDot_omega_3,
      SEqDot_omega_4; // quaternion rate from gyroscopes elements
  ...
  SEqHatDot_1 =
      J_14or21 * f_2 - J_11or24 * f_1 - J_41 * f_4 - J_51 * f_5 + J_61 * f_6;
  SEqHatDot_2 = J_12or23 * f_1 + J_13or22 * f_2 - J_32 * f_3 + J_42 * f_4 +
                J_52 * f_5 + J_62 * f_6;
  SEqHatDot_3 = J_12or23 * f_2 - J_33 * f_3 - J_13or22 * f_1 - J_43 * f_4 +
                J_53 * f_5 + J_63 * f_6;
  SEqHatDot_4 =
      J_14or21 * f_1 + J_11or24 * f_2 - J_44 * f_4 - J_54 * f_5 + J_64 * f_6;
  ...
  h_x = twom_x * (0.5f - SEq_3 * SEq_3 - SEq_4 * SEq_4) +
        twom_y * (SEq_2SEq_3 - SEq_1SEq_4) + twom_z * (SEq_2SEq_4 + SEq_1SEq_3);
