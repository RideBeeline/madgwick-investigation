...
import numpy as np
cimport numpy as np
from adafruit_madgwick cimport Adafruit_Madgwick
...
cdef class Adafruit(Base):
    """Wrapper around the Adafruit_Madgwick class."""

    cdef Adafruit_Madgwick c_ada

    def __cinit__(self):
        self.c_ada = Adafruit_Madgwick()
    ...
    def set_beta(self, beta):
        self.c_ada.beta = beta
    
    def set_freq(self, freq):
        self.c_ada.invSampleFreq = 1/freq
    ...
    def _run_updates(self, np.ndarray[DTYPE_t, ndim=2] acc, 
        ...
        for s in range(samples):
            ...
            self.c_ada.update(gyr[s,0] * rad2deg, gyr[s,1] * rad2deg, gyr[s,2] * rad2deg, 
