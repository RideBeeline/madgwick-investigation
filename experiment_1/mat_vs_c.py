...
m_Q, beta, freq, times, acc, gyr, mag = io.load_hdf5(
    mat.example_data_filename, load_imu_data=True)

# Calculate yaw values for matlab quaternions.
m_yaw = ot.q_to_aero_yaw(m_Q)

# Run same imu data through Madgwick's c code
mad_c = cpp.MadgwickOriginal(beta=beta, freq=freq)
c_Q = mad_c.update(acc, gyr, mag)
c_yaw = ot.q_to_aero_yaw(c_Q)

# Calculate differences in quaternion
...
fig.suptitle("Madgwick's implementations from x-io", fontsize=16)
...
ax0.text(0, -90, rf'$\beta={beta:0.2f},\ f={freq:.1f} \mathrm{{Hz}}$',
         fontsize=12, bbox=dict(boxstyle='round', fc='white', ec='lightgray'))

ax0.legend()

ax0.set_yticks([-90, -45, 0, 45, 90, 135])
...
plt.savefig("./exp1_mat_vs_c.png", transparent=False)
